...
$$\begin{eqnarray}
\widehat{\mbox{Preis}}&=&b_0 + b_1\cdot \mbox{Alter}\\
\end{eqnarray}$$
...
* Quantitative Übereinstimmung des Koeffizienten $b_1=`r round(get_regression_table(reg_auto)$estimate[2],2)`$ mit `obs_slope` aus der **infer**-Pipe

* Was ist mit den weiteren Spalten?
  * Standardfehler der Punktschätzer
  * t-Wert der Punktschätzer
  * p-Wert zum t-Test $(H_0: \beta_0=0$, $H_A: \beta_1\neq0)$ 
...
* Koeffizient $b_1=`r round(get_regression_table(reg_auto)$estimate[2],2)`$ (Steigungsparameter) minimiert für die vorliegende Stichprobe die Fehlerquadratsumme.

* Für alternative Stichprobe (aus derselben Grundgesamtheit) würde höchstwahrscheinlich ein anderer Wert für $b_1$ die Fehlerquadratsumme minimieren (`r fa('circle-right')` **Stichprobenvariabilität**).
...
$$\mbox{SE}_{b_1}=\frac{\frac{s_y}{s_x}\cdot\sqrt{1-r^2}}{\sqrt{n-2}}$$

* Teststatistik

$$t=\frac{b_1-\beta_1}{\mbox{SE}_{b_1}}$$
bzw. unter der Annahme, dass $H_0: \beta_1=0$  wahr ist: $t=\frac{b_1}{\mbox{SE}_{b_1}}$
...
* Berechnung Regressionskoeffizient $b_1$ mittels $b_1 = \frac{s_{X, Y}}{s^2_X}$
...
$$t=\frac{b_1-\beta_1}{\mbox{SE}_{b_1}} \sim t(n-2)$$
]
...
$H_A$, on the other hand, assumes that the population slope $\beta_1$ is not 0, meaning it could be either positive or negative. This suggests either a positive or negative relationship between teaching and "beauty" scores. 
...
* the standard error of $b_1 depends on
...
$n$ increases, the standard error $\mbox{SE}_{b_1}$ decreases

* Just **as we demonstrated** in Subsection 7.3.3 when **using shovels** with $n = 25$, $50$, and $100$ slots, the **amount of sampling variation of the fitted slope $b_1$ will depend on the sample size  
$n$
...
* **Exakt**: Bei wiederholter Stichprobenziehung (aus der Grundgesamtheit!) können wir erwarten, dass 95% der jeweiligen Konfidenzintervalle den  wahren Koeffizienten $\beta_1$ beinhalten. 

* Unser Konfidenzintervall für den Schätzer $b_1$ (`r lo_ci`; `r up_ci`)...
  * ...gehört entweder zu den 95% der Konfidenzintervalle, die den wahren Wert von $\beta_1$ umfassen, sodass dieser nicht Null sein kann.
  * ...oder es gehört zu den 5% der Konfidenzintervalle, die den wahren Wert nicht umfassen. Dann könnte $\beta_1$ mit geringer Wahrscheinlichkeit auch Null sein.
  
* Kurzform (**nicht exakt**): "Wir sind 95% "zuversichtlich" (*confident*), dass der wahre Koeffizient $\beta_1$ zwischen `r lo_ci` und  `r up_ci` liegt." (vgl. `r Cite(bib,"ismay_statistical_2024")`, Abschnitt 10.2.4)
]
]
...
$\beta_1$ (Steigung des Regressors X)
...
$$\begin{eqnarray}
\widehat{\mbox{Preis}}&=&b_0 + b_1\cdot \mbox{Alter} + b_2\cdot \mbox{km}\\
\end{eqnarray}$$
