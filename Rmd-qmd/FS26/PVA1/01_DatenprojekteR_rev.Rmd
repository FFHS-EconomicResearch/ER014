...
Im www finden sich viele Webseiten, die Daten zur Verfügung stellen, bspw. finden sich Daten zu Airbnb-Unterkünften in verschiedenen Städten auf [insideairbnb](http://insideairbnb.com/get-the-data).
...
Beispiel: Import von Airbnb-Daten für Zürich:

```{r,echo=TRUE}
### Daten importieren ----
tbl_rbnb <- read_csv("https://data.insideairbnb.com/switzerland/z%C3%BCrich/zurich/2024-06-29/data/listings.csv.gz")
```

### Webscraping und APIs

Nicht immer sind Daten auf Webseiten komfortabel als Dateidownload verfügbar. Wer die Daten nicht händisch aus der Webseite in eine Tabelle übertragen möchte, kann die Webseiten automatisiert auslesen (sog. webscraping).

Hierfür stehen auf `r fa('r-project')` die Pakete **httr** und seit 2023 [**httr2**](https://httr2.r-lib.org/) zur Verfügung. Die Bedienung setzt aber einige Vorkenntnisse zur Architektur von Webseiten voraus, die den Rahmen dieses Kurses sprengen.

Für einige Webseiten finden sich im CRAN aber auch spezielle Pakete, die das Webscraping spezifischer Seiten mit vergleichsweise einfacher Syntax ermöglichen. So können bspw. Fußballfans Daten zu verschiedenen Ligen mit dem Paket [worldfootballR] relativ komfortabel importieren.
...
Eine der Webseiten, die mit dem Paket **worldfootballR** ausgelesen werden kann ist bspw. [transfermarkt.org](www.transfermarkt.de), die Daten zu den deutschen Fußball-Ligen bereitstellt. 

Beispiel: Import der Bundesligaspieltage der Saison 2023:
...
df_Buli <- tm_matchday_table(country_name="Germany", start_year="2023", matchday=33)
## Kurzer Einblick ----
head(df_Buli)
...
APIs sind Programmierschnittstellen, die vom Anbieter bereitgestellt werden, bspw. um Daten zugänglich zu machen. Beispielweise stellen viele Statistische Ämter in Europa APIs für den Download amtlicher Daten zur Verfügung. So gibt es APIs von Eurostat, dem Schweizer Bundesamt für Statistik (BFS) oder auch dem Statistischen Bundesamt Deutschlands.

Ähnlich wie beim webscraping setzt die Nutzung von APIs einige Programmierkenntnisse voraus, die den Rahmen dieses Kurses übersteigen. Jedoch gibt es im CRAN diverse Pakete, die eine Nutzung spezieller APIs auch für Anfänger ermöglicht.

So bietet das Paket [**BFS**](https://cran.r-project.org/web/packages/BFS/readme/README.html), die Möglichkeit, Daten des Schweizer Bundesamts für Statistik direkt in `r fa('r-project')` zu importieren. Das Paket **wiesbaden** steht für den Import von Daten des Statistischen Bundesamts Deutschlands zur Verfügung.

Beispiel: Datenimport mit dem **wiesbaden**-Paket:
...
## Quarto und RMarkdown

* Quarto und RMarkdown bieten die Möglichkeit Analysen, Visualisierungen und Berichte flexibel in einem integrierten und reproduzierbaren Workflow zu erstellen.
